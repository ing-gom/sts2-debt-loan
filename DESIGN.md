...
- **막 제한 폐지** — 언제든 빌릴 수 있다(`ActAllowsLoan` → 항상 true). 청산이 유물을 없애지 않고 신용도가 런 내내
  쌓이는 구조가 되면서 "대출은 런 초반의 일회성 결정"이라는 전제 자체가 사라졌다.
  ★**상환은 빌린 그 상점에서는 불가**(`CanRepayHere`) — 수수료 20%를 물고도 즉시 되갚기를 반복하면
  순 골드 200으로 최고 보상(누적 1200)까지 갈 수 있어서다. 다음 상점까지 걷게 하면 노드 이자가 붙어
  루프가 스스로 비싸진다.
- **★인출 횟수 제한(`MaxLoanDraws`=3)**: 한 대출에서 골드를 나눠 받을 수 있는 횟수(최초 대출 + 추가 인출
  ...
  - **청산 시 리셋** — `ApplyRepay`가 `LoanDraws=0` + `Active=false`. ★record는 **사라지지 않는다**(유물·결제
    카드·누적 상환액 유지). 그래서 `rec.Active` 를 "뺚이 있다"로 읽는 모든 호출부가 의미를 갖는다 —
    열어둔 채 두면 `CanLoanCover` 의 "톱업은 빌린 상점에서만" 규칙이 영구 적용돼 **다음 상점부터
    영영 대출 불가**가 된다(실제로 잡은 회귀).
