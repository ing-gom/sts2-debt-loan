...
internal static class DebtLoanLoc
{
    ...
    private static readonly Dictionary<string, string> _cardsEng = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "Gain [b]{CalculatedBlock}[/b] [gold]Block[/gold] — a base amount plus [b]{CalculationExtra}[/b] per [gold]Receipt[/gold] you hold.",
        ["INVOICE_CARD.title"] = "Invoice",
        ["INVOICE_CARD.description"] = "Deal [b]{Damage}[/b] damage [b]{CalculatedHits}[/b] times — once, plus once per [gold]Receipt[/gold] you hold.",
    ...
    private static readonly Dictionary<string, string> _cardsKor = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "[gold]블록[/gold] [b]{CalculatedBlock}[/b]을 얻습니다 — 기본값에 보유한 [gold]영수증[/gold] 1개당 [b]{CalculationExtra}[/b]를 더합니다.",
        ["INVOICE_CARD.title"] = "청구서",
        ["INVOICE_CARD.description"] = "적에게 [b]{Damage}[/b]의 피해를 [b]{CalculatedHits}[/b]번 줍니다 — 기본 1타에 보유한 [gold]영수증[/gold] 1개당 1타를 더합니다.",
    ...
    private static readonly Dictionary<string, string> _cardsJpn = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "[gold]ブロック[/gold]を[b]{CalculatedBlock}[/b]得る — 基本値に加えて、所持している[gold]レシート[/gold]1枚につき[b]{CalculationExtra}[/b]。",
        ["INVOICE_CARD.title"] = "請求書",
        ["INVOICE_CARD.description"] = "[b]{Damage}[/b]のダメージを[b]{CalculatedHits}[/b]回与える — 1回に加えて、所持している[gold]レシート[/gold]1枚につき1回。",
    ...
    private static readonly Dictionary<string, string> _cardsZhs = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "获得 [b]{CalculatedBlock}[/b] 点[gold]格挡[/gold]——基础值外加你持有的每张[gold]收据[/gold]提供 [b]{CalculationExtra}[/b]。",
        ["INVOICE_CARD.title"] = "账单",
        ["INVOICE_CARD.description"] = "造成 [b]{Damage}[/b] 点伤害，共 [b]{CalculatedHits}[/b] 次——一次，外加你持有的每张[gold]收据[/gold]再打一次。",
    ...
    private static readonly Dictionary<string, string> _cardsDeu = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "Erhalte [b]{CalculatedBlock}[/b] [gold]Block[/gold] — ein Grundwert plus [b]{CalculationExtra}[/b] pro [gold]Beleg[/gold], den du hältst.",
        ["INVOICE_CARD.title"] = "Rechnung",
        ["INVOICE_CARD.description"] = "Füge [b]{Damage}[/b] Schaden [b]{CalculatedHits}[/b]-mal zu — einmal, plus einmal pro [gold]Beleg[/gold], den du hältst.",
    ...
    private static readonly Dictionary<string, string> _cardsFra = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "Gagne [b]{CalculatedBlock}[/b] [gold]Armure[/gold] — un montant de base plus [b]{CalculationExtra}[/b] par [gold]reçu[/gold] que tu possèdes.",
        ["INVOICE_CARD.title"] = "Facture",
        ["INVOICE_CARD.description"] = "Inflige [b]{Damage}[/b] dégâts [b]{CalculatedHits}[/b] fois — une fois, plus une fois par [gold]reçu[/gold] que tu possèdes.",
    ...
    private static readonly Dictionary<string, string> _cardsSpa = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "Gana [b]{CalculatedBlock}[/b] de [gold]Bloqueo[/gold] — una cantidad base más [b]{CalculationExtra}[/b] por cada [gold]recibo[/gold] que tengas.",
        ["INVOICE_CARD.title"] = "Factura",
        ["INVOICE_CARD.description"] = "Inflige [b]{Damage}[/b] de daño [b]{CalculatedHits}[/b] veces — una vez, más una vez por cada [gold]recibo[/gold] que tengas.",
    ...
    private static readonly Dictionary<string, string> _cardsIta = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "Ottieni [b]{CalculatedBlock}[/b] [gold]Blocco[/gold] — un valore base più [b]{CalculationExtra}[/b] per ogni [gold]ricevuta[/gold] che possiedi.",
        ["INVOICE_CARD.title"] = "Fattura",
        ["INVOICE_CARD.description"] = "Infliggi [b]{Damage}[/b] danni [b]{CalculatedHits}[/b] volte — una volta, più una volta per ogni [gold]ricevuta[/gold] che possiedi.",
    ...
    private static readonly Dictionary<string, string> _cardsPol = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "Zyskaj [b]{CalculatedBlock}[/b] [gold]Bloku[/gold] — wartość bazowa plus [b]{CalculationExtra}[/b] za każdy posiadany [gold]paragon[/gold].",
        ["INVOICE_CARD.title"] = "Faktura",
        ["INVOICE_CARD.description"] = "Zadaj [b]{Damage}[/b] obrażeń [b]{CalculatedHits}[/b] razy — raz, plus raz za każdy posiadany [gold]paragon[/gold].",
    ...
    private static readonly Dictionary<string, string> _cardsPtb = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "Ganhe [b]{CalculatedBlock}[/b] de [gold]Proteção[/gold] — um valor base mais [b]{CalculationExtra}[/b] por [gold]recibo[/gold] que você possui.",
        ["INVOICE_CARD.title"] = "Fatura",
        ["INVOICE_CARD.description"] = "Cause [b]{Damage}[/b] de dano [b]{CalculatedHits}[/b] vezes — uma vez, mais uma vez por [gold]recibo[/gold] que você possui.",
    ...
    private static readonly Dictionary<string, string> _cardsRus = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "Получите [b]{CalculatedBlock}[/b] [gold]Защиты[/gold] — базовое значение плюс [b]{CalculationExtra}[/b] за каждый имеющийся [gold]чек[/gold].",
        ["INVOICE_CARD.title"] = "Счёт",
        ["INVOICE_CARD.description"] = "Нанесите [b]{Damage}[/b] урона [b]{CalculatedHits}[/b] раз — один раз, плюс ещё раз за каждый имеющийся [gold]чек[/gold].",
    ...
    private static readonly Dictionary<string, string> _cardsTha = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "รับ[gold]บล็อก[/gold] [b]{CalculatedBlock}[/b] — ค่าพื้นฐานบวก [b]{CalculationExtra}[/b] ต่อ[gold]ใบเสร็จ[/gold]ที่คุณถืออยู่แต่ละใบ",
        ["INVOICE_CARD.title"] = "ใบแจ้งหนี้",
        ["INVOICE_CARD.description"] = "สร้างความเสียหาย [b]{Damage}[/b] จำนวน [b]{CalculatedHits}[/b] ครั้ง — หนึ่งครั้ง บวกอีกหนึ่งครั้งต่อ[gold]ใบเสร็จ[/gold]ที่คุณถืออยู่แต่ละใบ",
    ...
    private static readonly Dictionary<string, string> _cardsTur = new()
    {
        ...
        ["SETTLEMENT_CARD.description"] = "[b]{CalculatedBlock}[/b] [gold]Blok[/gold] kazan — temel bir miktar artı elinde tuttuğun her [gold]makbuz[/gold] için [b]{CalculationExtra}[/b].",
        ["INVOICE_CARD.title"] = "Fatura",
        ["INVOICE_CARD.description"] = "[b]{Damage}[/b] hasarı [b]{CalculatedHits}[/b] kez ver — bir kez, artı elinde tuttuğun her [gold]makbuz[/gold] için bir kez daha.",
