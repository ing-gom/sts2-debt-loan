...
/// choose() on {cards} (the current tier 1..4) to name the current highest-stage curse — so the description
/// CHANGES as the debt escalates. Dunning uses {draw}/{play}/{plate}; the Forced Collection uses
/// {hp}/{principal}. [gold]…[/gold] / [b]…[/b] BBCode + the placeholders + the choose(...) syntax are kept
...
internal static class DebtLoanLoc
{
    ...
    private static readonly Dictionary<string, Row> ByLang = new()
    {
        ["eng"] = new("Merchant's Ledger",
            "Borrowed [gold]{borrowed} Gold[/gold]. Paid [gold]{paid} Gold[/gold] so far.\nEach combat injects Debt curses — current stage: [b]{cards:choose(1|2|3|4):Dunning|Delinquency|Seizure|Bad Credit|Dunning}[/b]. The longer you owe, the worse it gets.\nRepay the debt at a shop to remove this relic.",
        ...
        ["kor"] = new("상인의 장부",
            "빌린 금액 [gold]{borrowed} 골드[/gold]. 지금까지 지불 [gold]{paid} 골드[/gold].\n전투마다 빚 저주가 주입됩니다 — 현재 최고 단계: [b]{cards:choose(1|2|3|4):빚 독촉|연체|차압|신용 불량|빚 독촉}[/b]. 오래 갚지 않을수록 악화됩니다.\n상점에서 빚을 갚으면 이 유물이 제거됩니다.",
            "모든 서명은 작은 항복이다.",
        ...
        ["zhs"] = new("商人的账簿",
            "已借入 [gold]{borrowed} 金币[/gold]。已偿还 [gold]{paid} 金币[/gold]。\n每场战斗会注入债务诅咒——当前最高阶段：[b]{cards:choose(1|2|3|4):催债|拖欠|扣押|信用不良|催债}[/b]。拖欠越久，情况越糟。\n在商店还清债务即可移除此遗物。",
            "每一个签名都是一次小小的屈服。",
        ...
        ["deu"] = new("Händlerbuch",
            "Geliehen: [gold]{borrowed} Gold[/gold]. Zurückgezahlt: [gold]{paid} Gold[/gold].\nJeder Kampf schleust Schulden-Flüche ein — aktuelle Stufe: [b]{cards:choose(1|2|3|4):Mahnung|Verzug|Pfändung|Zahlungsunfähig|Mahnung}[/b]. Je länger du schuldest, desto schlimmer.\nZahle die Schuld in einem Laden zurück, um dieses Relikt zu entfernen.",
        ...
        ["fra"] = new("Grand livre du marchand",
            "Emprunté : [gold]{borrowed} or[/gold]. Remboursé : [gold]{paid} or[/gold].\nChaque combat injecte des malédictions de Dette — stade actuel : [b]{cards:choose(1|2|3|4):Relance|Défaut|Saisie|Insolvabilité|Relance}[/b]. Plus tu tardes, pire c'est.\nRembourse la dette dans une boutique pour retirer cette relique.",
        ...
        ["spa"] = new("Libro del mercader",
            "Prestado: [gold]{borrowed} de oro[/gold]. Pagado: [gold]{paid} de oro[/gold].\nCada combate inyecta maldiciones de Deuda — etapa actual: [b]{cards:choose(1|2|3|4):Reclamación|Morosidad|Embargo|Insolvencia|Reclamación}[/b]. Cuanto más debas, peor será.\nSalda la deuda en una tienda para eliminar esta reliquia.",
        ...
        ["ita"] = new("Registro del mercante",
            "Preso in prestito: [gold]{borrowed} Oro[/gold]. Pagato: [gold]{paid} Oro[/gold].\nOgni combattimento inserisce maledizioni di Debito — stadio attuale: [b]{cards:choose(1|2|3|4):Sollecito|Morosità|Pignoramento|Insolvenza|Sollecito}[/b]. Più tardi, peggio è.\nSalda il debito in un negozio per rimuovere questo cimelio.",
        ...
        ["pol"] = new("Księga kupca",
            "Pożyczono: [gold]{borrowed} złota[/gold]. Spłacono: [gold]{paid} złota[/gold].\nKażda walka dodaje klątwy Długu — obecny etap: [b]{cards:choose(1|2|3|4):Ponaglenie|Zaległość|Zajęcie|Niewypłacalność|Ponaglenie}[/b]. Im dłużej zwlekasz, tym gorzej.\nSpłać dług w sklepie, aby usunąć ten relikt.",
        ...
        ["ptb"] = new("Livro-razão do mercador",
            "Emprestado: [gold]{borrowed} de Ouro[/gold]. Pago: [gold]{paid} de Ouro[/gold].\nCada combate injeta maldições de Dívida — estágio atual: [b]{cards:choose(1|2|3|4):Cobrança|Inadimplência|Penhora|Crédito Ruim|Cobrança}[/b]. Quanto mais você deve, pior fica.\nQuite a dívida em uma loja para remover esta relíquia.",
        ...
        ["rus"] = new("Гроссбух торговца",
            "Взято в долг: [gold]{borrowed} золота[/gold]. Выплачено: [gold]{paid} золота[/gold].\nКаждый бой добавляет проклятия Долга — текущая стадия: [b]{cards:choose(1|2|3|4):Взыскание|Просрочка|Арест|Неплатёжеспособность|Взыскание}[/b]. Чем дольше долг, тем хуже.\nПогасите долг в магазине, чтобы убрать эту реликвию.",
        ...
        ["tha"] = new("บัญชีของพ่อค้า",
            "ยืมมา [gold]{borrowed} ทอง[/gold] จ่ายไปแล้ว [gold]{paid} ทอง[/gold]\nการต่อสู้แต่ละครั้งจะใส่คำสาปหนี้ — ระดับปัจจุบัน: [b]{cards:choose(1|2|3|4):ทวงหนี้|ค้างชำระ|ยึดทรัพย์|เครดิตเสีย|ทวงหนี้}[/b] ยิ่งค้างนานยิ่งแย่\nชำระหนี้ที่ร้านค้าเพื่อนำวัตถุโบราณนี้ออก",
        ...
        ["tur"] = new("Tüccarın Defteri",
            "Borç alınan: [gold]{borrowed} Altın[/gold]. Ödenen: [gold]{paid} Altın[/gold].\nHer savaş Borç lanetleri ekler — mevcut aşama: [b]{cards:choose(1|2|3|4):İhtar|Temerrüt|Haciz|Kredi İflası|İhtar}[/b]. Borç uzadıkça kötüleşir.\nBu kalıntıyı kaldırmak için borcu bir dükkânda öde.",
