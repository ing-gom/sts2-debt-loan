...
///
/// The relic description is a per-relic template ({borrowed}/{paid}/{cards}). Dunning uses {draw}/{play}
/// ({draw} = the passive turn-end collection); the Forced Collection uses {hp}/{principal} (its DynamicVars,
/// set from the collection level). [gold]…[/gold] BBCode and the placeholders are kept verbatim — only the
...
internal static class DebtLoanLoc
{
    ...
    private static readonly Dictionary<string, Row> ByLang = new()
    {
        ["eng"] = new("Merchant's Ledger",
            "Borrowed [gold]{borrowed} Gold[/gold]. Paid [gold]{paid} Gold[/gold] so far.\nEach combat injects [b]{cards}[/b] Debt card(s) into your draw pile — escalating as time passes.\nRepay the debt at a shop to remove this relic.",
            "Every signature is a small surrender.",
            "Dunning", "Ethereal. At the end of your turn, if it's still in your hand, it collects [gold]{draw} Gold[/gold] and vanishes. Or play it to pay [gold]{play} Gold[/gold] and clear the loan faster.",
        ...
        ["kor"] = new("상인의 장부",
            "빌린 금액 [gold]{borrowed} 골드[/gold]. 지금까지 지불 [gold]{paid} 골드[/gold].\n전투마다 빚 카드 [b]{cards}[/b]장이 뽑을 더미에 주입됩니다 — 시간이 지날수록 강해집니다.\n상점에서 빚을 갚으면 이 유물이 제거됩니다.",
            "모든 서명은 작은 항복이다.",
            "빚 독촉", "휘발. 턴 종료 시 손에 있으면 [gold]{draw} 골드[/gold]를 징수하고 사라집니다. 또는 플레이하여 [gold]{play} 골드[/gold]를 내고 빚을 더 빨리 갚습니다.",
        ...
        ["jpn"] = new("商人の元帳",
            "借入 [gold]{borrowed} ゴールド[/gold]。これまでの支払い [gold]{paid} ゴールド[/gold]。\n戦闘ごとに借金カードが[b]{cards}[/b]枚、ドロー山札に加わる — 時間経過で激しくなる。\nショップで借金を返済すると、このレリックは取り除かれる。",
            "署名はすべて、小さな降伏だ。",
            "督促", "エセリアル。ターン終了時、手札にあれば[gold]{draw} ゴールド[/gold]を徴収して消滅する。またはプレイして[gold]{play} ゴールド[/gold]を支払い、借金をより早く返済する。",
        ...
        ["zhs"] = new("商人的账簿",
            "已借入 [gold]{borrowed} 金币[/gold]。已偿还 [gold]{paid} 金币[/gold]。\n每场战斗会将 [b]{cards}[/b] 张债务牌注入你的抽牌堆——随时间不断升级。\n在商店还清债务即可移除此遗物。",
            "每一个签名都是一次小小的屈服。",
            "催债", "虚无。回合结束时若仍在手牌中，征收 [gold]{draw} 金币[/gold] 后消失。或打出它，支付 [gold]{play} 金币[/gold] 以更快偿还贷款。",
        ...
        ["deu"] = new("Händlerbuch",
            "Geliehen: [gold]{borrowed} Gold[/gold]. Zurückgezahlt: [gold]{paid} Gold[/gold].\nJeder Kampf schleust [b]{cards}[/b] Schulden-Karte(n) in deinen Nachziehstapel — mit der Zeit immer schlimmer.\nZahle die Schuld in einem Laden zurück, um dieses Relikt zu entfernen.",
            "Jede Unterschrift ist eine kleine Kapitulation.",
            "Mahnung", "Ätherisch. Am Ende deiner Runde zieht sie, wenn noch auf der Hand, [gold]{draw} Gold[/gold] ein und verschwindet. Oder spiele sie, um [gold]{play} Gold[/gold] zu zahlen und die Schuld schneller zu tilgen.",
        ...
        ["fra"] = new("Grand livre du marchand",
            "Emprunté : [gold]{borrowed} or[/gold]. Remboursé : [gold]{paid} or[/gold].\nChaque combat injecte [b]{cards}[/b] carte(s) Dette dans ta pioche — de pire en pire avec le temps.\nRembourse la dette dans une boutique pour retirer cette relique.",
            "Chaque signature est une petite reddition.",
            "Relance", "Éthérée. À la fin de ton tour, si elle est encore en main, elle prélève [gold]{draw} or[/gold] et disparaît. Ou joue-la pour payer [gold]{play} or[/gold] et rembourser la dette plus vite.",
        ...
        ["spa"] = new("Libro del mercader",
            "Prestado: [gold]{borrowed} de oro[/gold]. Pagado: [gold]{paid} de oro[/gold].\nCada combate inyecta [b]{cards}[/b] carta(s) de Deuda en tu mazo de robo — peor con el tiempo.\nSalda la deuda en una tienda para eliminar esta reliquia.",
            "Cada firma es una pequeña rendición.",
            "Reclamación", "Etérea. Al final de tu turno, si sigue en tu mano, recauda [gold]{draw} de oro[/gold] y desaparece. O juégala para pagar [gold]{play} de oro[/gold] y saldar la deuda más rápido.",
        ...
        ["ita"] = new("Registro del mercante",
            "Preso in prestito: [gold]{borrowed} Oro[/gold]. Pagato: [gold]{paid} Oro[/gold].\nOgni combattimento inserisce [b]{cards}[/b] carta/e Debito nel tuo mazzo di pesca — sempre peggio col tempo.\nSalda il debito in un negozio per rimuovere questo cimelio.",
            "Ogni firma è una piccola resa.",
            "Sollecito", "Eterea. Alla fine del turno, se è ancora in mano, riscuote [gold]{draw} Oro[/gold] e svanisce. Oppure giocala per pagare [gold]{play} Oro[/gold] e saldare il debito più in fretta.",
        ...
        ["pol"] = new("Księga kupca",
            "Pożyczono: [gold]{borrowed} złota[/gold]. Spłacono: [gold]{paid} złota[/gold].\nKażda walka dodaje [b]{cards}[/b] kart(y) Długu do talii dobierania — z czasem coraz gorzej.\nSpłać dług w sklepie, aby usunąć ten relikt.",
            "Każdy podpis to mała kapitulacja.",
            "Ponaglenie", "Ulotna. Na końcu tury, jeśli wciąż jest w ręce, pobiera [gold]{draw} złota[/gold] i znika. Albo zagraj ją, aby zapłacić [gold]{play} złota[/gold] i szybciej spłacić dług.",
        ...
        ["ptb"] = new("Livro-razão do mercador",
            "Emprestado: [gold]{borrowed} de Ouro[/gold]. Pago: [gold]{paid} de Ouro[/gold].\nCada combate injeta [b]{cards}[/b] carta(s) de Dívida no seu monte de compra — pior com o tempo.\nQuite a dívida em uma loja para remover esta relíquia.",
            "Cada assinatura é uma pequena rendição.",
            "Cobrança", "Etérea. No fim do seu turno, se ainda estiver na mão, cobra [gold]{draw} de Ouro[/gold] e desaparece. Ou jogue-a para pagar [gold]{play} de Ouro[/gold] e quitar a dívida mais rápido.",
        ...
        ["rus"] = new("Гроссбух торговца",
            "Взято в долг: [gold]{borrowed} золота[/gold]. Выплачено: [gold]{paid} золота[/gold].\nКаждый бой добавляет [b]{cards}[/b] карт(ы) Долга в колоду добора — со временем всё хуже.\nПогасите долг в магазине, чтобы убрать эту реликвию.",
            "Каждая подпись — маленькая капитуляция.",
            "Взыскание", "Эфемерная. В конце хода, если она ещё в руке, взимает [gold]{draw} золота[/gold] и исчезает. Или разыграйте её, чтобы заплатить [gold]{play} золота[/gold] и быстрее погасить долг.",
        ...
        ["tha"] = new("บัญชีของพ่อค้า",
            "ยืมมา [gold]{borrowed} ทอง[/gold] จ่ายไปแล้ว [gold]{paid} ทอง[/gold]\nการต่อสู้แต่ละครั้งจะใส่การ์ดหนี้ [b]{cards}[/b] ใบลงในกองจั่ว — รุนแรงขึ้นเมื่อเวลาผ่านไป\nชำระหนี้ที่ร้านค้าเพื่อนำวัตถุโบราณนี้ออก",
            "ทุกลายเซ็นคือการยอมจำนนเล็กๆ",
            "ทวงหนี้", "อีเทอเรียล เมื่อจบเทิร์นหากยังอยู่ในมือ จะเก็บ [gold]{draw} ทอง[/gold] แล้วหายไป หรือเล่นเพื่อจ่าย [gold]{play} ทอง[/gold] และชำระหนี้เร็วขึ้น",
        ...
        ["tur"] = new("Tüccarın Defteri",
            "Borç alınan: [gold]{borrowed} Altın[/gold]. Ödenen: [gold]{paid} Altın[/gold].\nHer savaş çekme destene [b]{cards}[/b] Borç kartı ekler — zamanla giderek kötüleşir.\nBu kalıntıyı kaldırmak için borcu bir dükkânda öde.",
            "Her imza küçük bir teslimiyettir.",
            "İhtar", "Uçucu. Turunun sonunda hâlâ elindeyse [gold]{draw} Altın[/gold] tahsil eder ve kaybolur. Ya da oyna: [gold]{play} Altın[/gold] öde ve borcu daha hızlı kapat.",
